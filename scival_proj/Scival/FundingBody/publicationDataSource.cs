...
{
    public partial class publicationDataSource : UserControl
    {
        DataSet dsItems;
        ...
        public publicationDataSource(FundingBody frm)
        {
            InitializeComponent();
            LoadinitialVale();
            m_parent = frm;
        ...
        private void LoadinitialVale()
        {
            try
            {
                ...
                ddl_Frequency.SelectedIndex = 0;//Schema5.0
                BindGrid();
            }
            catch (Exception ex)
            {
        ...
        public void BindGrid()
        {
            try
            {
                Int64 WFID = Convert.ToInt64(SharedObjects.WorkId);
                pagemode = 3;
                dsItems = FundingBodyDataOperations.GetPublicationsSource(Convert.ToInt64(SharedObjects.WorkId), pagemode);
        ...
        private void grdoppottunities_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 10)
            {
                try
                {
                    ...
                    DataSet dsresult = FundingBodyDataOperations.updatePublicationSource5(WFID, pagemode, Convert.ToInt64(dsItems.Tables["ItemListDDLDisplay"].Rows[rowindex]["Pub_SOURCE_ID"]), ddlStatus.Text.ToString(), "", txtUrl.Text, ddlLangContextName.SelectedValue.ToString(), "", "", "", "", "");
