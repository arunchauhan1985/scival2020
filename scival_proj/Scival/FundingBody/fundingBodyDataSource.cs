...
{
    public partial class fundingBodyDataSource : UserControl
    {
        DataSet dsItems;
        ...
        public fundingBodyDataSource(FundingBody frm)
        {
            InitializeComponent();
            LoadinitialVale();
            m_parent = frm;
        ...
        public void BindGrid()
        {
            try
            {
                Int64 WFID = Convert.ToInt64(SharedObjects.WorkId);
                pagemode = 3;
                dsItems = FundingBodyDataOperations.GetFundingbodiesSource(Convert.ToInt64(SharedObjects.WorkId), pagemode);
        ...
        private void grdoppottunities_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 10)
            {
                try
                {
                    ...
                    DataSet dsresult = FundingBodyDataOperations.updateFundingbodySource5(WFID, pagemode, Convert.ToInt64(dsItems.Tables["ItemListDDLDisplay"].Rows[rowindex]["FB_SOURCE_ID"]), ddlStatus.Text.ToString(), "", txtUrl.Text, ddlLangContextName.SelectedValue.ToString(), "", "", "", "", "");
