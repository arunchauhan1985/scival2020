    ...
    public partial class Contact : UserControl
    {
        ...
        private void LoadinitialVale()
        {
            try
            {
                ...
                DataSet dsItems = AwardDataOperations.GetContactsList(SharedObjects.WorkId, pagemode);
                ...
                else
                {
                    try
                    {
                        ...
                        DataSet dsresult = AwardDataOperations.SaveAndDeleteContactsLIst(WFID, pagemode, 0, 0, TextType.Text.Trim(), TextTitle.Text.Trim(), TextTelephone.Text.Trim(), TextFax.Text.Trim(), TextEmail.Text.Trim(), TextURL.Text.Trim(), TextLinkText.Text.Trim(), Country, TextRoom.Text.Trim(), TextStreet.Text.Trim(), state, TextCity.Text.Trim(), TextPostalCode.Text.Trim(), TextPrefix.Text.Trim(), TextGivenName.Text.Trim(), TextMiddleName.Text.Trim(), TextSurName.Text.Trim(), TextSuffix.Text.Trim());
                            ...
                            {
                                m_parent.GetProcess("Award Manager");
                            }
                            else
                            {
                                m_parent.GetProcess();
            ...
            try
            {
                ...
                if (dttempContact.Rows.Count > 0)
                {
                    if (e.KeyValue == 46)
                    {
                        ...
                        {
                            DataSet dsItems = AwardDataOperations.SaveAndDeleteContactsLIst(SharedObjects.WorkId, pagemode, 1, Convert.ToInt64(dttempContact.Rows[grdContact.SelectedCells[0].RowIndex]["contact_id"]), "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "");
                            ...
                            m_parent.GetProcess();
                            lblMsg.Visible = true;
                            lblMsg.Text = Convert.ToString(dsItems.Tables["ERRORCODE"].Rows[0][1]);
                        }
                    }
                }
                else
            ...
            if ((url_txtLinkUrl.Contains("http://") || (url_txtLinkUrl.Contains("https://") || (url_txtLinkUrl.Contains("www.")))))
            {
                ...
                else
                {
                    try
                    {
                        ...
                        string COUNTRY = Convert.ToString(DDLCOUNTRY.SelectedValue);
                        ...
                        DataSet dsresult = AwardDataOperations.SaveAndDeleteContactsLIst(WFID, pagemode, 2, Convert.ToInt64(dttempContact.Rows[rowindex]["contact_id"]), TYPE, TITLE, TELEPHONE, FAX, EMAIL, url, WEBSITE_TEXT, COUNTRY, ROOM, STREET, STATE, CITY, POSTALCODE, PREFIX, GIVENNAME, MIDDLENAME, SURNAME, SUFFIX);
